...
WORK_MOUNT = "/work"
LEVELS = ("base", "gui", "games", "chrome", "ai", "all")
LOCK_FILE = ROOT / ".build-lock"


def _level(base=False, gui=False, games=False, chrome=False, ai=False, all=False):
    """Return the level string based on which level is requested.
    
    One-of enumeration: base -> gui -> games -> chrome -> ai -> all
    """
    if all:
        return "all"
    if ai:
...
def _kas_args(level):
    ...
    Chain: base -> gui -> [games] -> [chrome] -> [ai] OR all
    """
    if level == "base":
        return "kas/base.yml"
    elif level == "gui":
