...
const containerStatus = defineTool({
	...
	async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "container-status", []);
...
const containerStart = defineTool({
	...
	parameters: Type.Object({}),
	async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "container-start", []);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const containerStop = defineTool({
	...
	parameters: Type.Object({}),
	async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "container-stop", []);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const containerExec = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvokeShort(ctx.pi, "bash", [`--command=${containerCmd}`]);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildStart = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvoke(ctx.pi, "build-start", [`--${level}`, "--detach"], 300_000);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildCheckout = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvoke(ctx.pi, "build-checkout", args, 300_000);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildStop = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		const args: string[] = [];
		if (params.force) args.push("--force");
		const r = await runInvokeShort(ctx.pi, "build-stop", args);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildStatus = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "build-status", [`--lines=${params.lines}`]);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildLogs = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		const level = params.level ?? "core";
		const logPath = `${ctx.pi.cwd}/build-${level}.log`;
		const r = await runInvokeShort(ctx.pi, "bash", [`--command=tail -n ${params.lines} "${logPath}"`]);
		if (!r.success) {
			return {
		...
		let status = "EXITED";
		try {
			const lockCheck = await runInvokeShort(ctx.pi, "bash", [`--command=cat .build-lock 2>/dev/null || echo "none"`]);
			...
			if (lock.level === level && lock.pid) {
				...
				const alive = await runInvokeShort(ctx.pi, "bash", [`--command=${aliveCmd}`]);
...
const buildLast = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "build-last", [`--lines=${params.lines}`]);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildShell = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvoke(ctx.pi, "shell", [`--${level}`, `--command=${params.command}`], 300_000);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildClean = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvokeShort(ctx.pi, "build-clean", args);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildRebuild = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		const r = await runInvoke(ctx.pi, "build-rebuild", [`--${level}`], 300_000);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildImages = defineTool({
	...
	parameters: Type.Object({}),
	async execute(_toolCallId, _params, _signal, _onUpdate, ctx) {
		const r = await runInvokeShort(ctx.pi, "images", []);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const buildFlash = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		if (params.force) args.push("--force");
		const r = await runInvoke(ctx.pi, "flash", args, 300_000);
		return { content: [{ type: "text", text: r.text }], details: { exit: r.success ? 0 : 1 } };
	},
});
...
const targetConnect = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		...
		try {
			const r = await sshExec(ctx.pi, "echo OK");
			if (r.includes("OK")) {
				return {
...
const targetExec = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		try {
			const r = await sshExec(ctx.pi, params.command);
...
const targetSudo = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		try {
			const r = await sshExec(ctx.pi, params.command, true);
			return { content: [{ type: "text", text: r }], details: { exit: 0 } };
		} catch (e) {
			return {
				content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : String(e)}` }],
...
const targetDocker = defineTool({
	...
	async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
		try {
			const r = await sshExec(ctx.pi, `docker ${params.command}`);
			return { content: [{ type: "text", text: r }], details: { exit: 0 } };
		} catch (e) {
			return {
				content: [{ type: "text", text: `Error: ${e instanceof Error ? e.message : String(e)}` }],
